...
export const attachSystemInfoListener = (windowContainingStore: BrowserWindow) => {
  ...
  if (isMac) {
    ...
    setTimeout(() => {
      // dnd module may take some time to init
      try {
...
export const attachWindowStoreListener = (
  trackedWindow: BrowserWindow,
  frameName: WindowFrameName,
  receivingWindow: BrowserWindow
) => {
  const sendInfo = () => {
    try {
    ...
    (_event: Electron.Event, channel: string, ...args: any[]) => {
      try {
        ...
        if (channel === WindowIpcTopic.REQUEST_WINDOW_INFO) {
          ...
          if (
            msg.backgroundThrottling !== undefined &&
            trackedWindow.webContents?.setBackgroundThrottling
          ) {
          ...
          if (msg.contentSize !== undefined) {
            trackedWindow.setContentSize(
              msg.contentSize.bounds.width,
              msg.contentSize.bounds.height,
              msg.contentSize.animate
            )
          }
        ...
        log(
          `WindowStore listener for ${frameName} couldn't handle message:`,
          args,
          'caused error',
          e
        )
